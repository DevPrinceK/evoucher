...
class OrganizerAndAppUserOnly(object):
    '''Decorator to allow access to both organizers and app users'''

    def __init__(self, original_method):
        self.original_method = original_method

    def __call__(self, request, *args,  **kwargs):
        if (request.user.is_authenticated) and (request.user.role == "ORGANIZER" or request.user.role == "APP_USER"):
...
class OrganizerAndRestaurantOnly(object):
